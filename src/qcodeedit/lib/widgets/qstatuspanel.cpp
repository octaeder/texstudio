...
bool QStatusPanel::paint(QPainter *p, QEditor *e)
{
	...
	if (c) {
		QString zoomInfo = e->getZoomInfo();
		if (e->document() && zoomInfo != "") {
			if (!showZoomTimer) {
qDebug()<<"create timer";
				showZoomTimer = new QTimer(this);
				showZoomTimer->start(1000);
				showZoomTimer->setSingleShot(true);
				connect(showZoomTimer, SIGNAL(timeout()), this, SLOT(clearZoomInfo()));
			}
			s = zoomInfo;		// #2371
		} else {
...
void QStatusPanel::clearZoomInfo()
{
qDebug()<<"clearZoomInfo";
	zoomInfo = QString();
